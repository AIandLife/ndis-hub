...
5. **对接大厅 `/board`**（2026-06-12 上线，Fable 提出的核心定位）：**网站本质=需求收割机**。聚合全行业「在找什么」（成员档案的需 + demands 表自助发布），任何人不入驻也能免费挂需求 → Terry 审核 → 公开展示。每条需求 = Terry 转介佣金的库存。demands 表镜像 BossLink；联系方式只存库不外显，公开读走 /api/demands 服务端过滤。
6. **对接=直连不强制进群**（Terry 2026-06-12 拍板：群里他没法逐个牵线）。「我能对接」→ ConnectModal 留称呼+微信 → 对方留过微信(自助需求)就当场互换直连；圈内成员(库里无联系方式)走圈主牵线。每次对接写 connection_requests + 邮件通知 Terry——**数据留底,人直连,线索归 Terry**。
